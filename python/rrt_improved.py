...
def sample_random_position(occupancy_grid):
  ...
  while not valid:
    ...
    position = occupancy_grid.get_position(rand_i * shape[0], rand_j * shape[1])
    valid = occupancy_grid.is_free(position)
...
def adjust_pose(node, final_position, occupancy_grid):
  ...
  a = node.position
  a_tang = node.direction
  b = final_position
  ab = a - b
  p = perpendicular(ab)
  p_norm = p / np.linalg.norm(p)

  a_norm = perpendicular(node.direction)
  b_norm = a_norm - 2 * np.dot(a_norm, p_norm) * p_norm
  b_tang = perpendicular(b_norm)

  # if the angle between a_tang and b_tang is pi/2 < angle < 3*pi/2, flip b
  if np.dot(a_tang, b_tang) / (np.linalg.norm(a_tang) * np.linalg.norm(b_tang)) < 0:
    b_tang = -b_tang

  yaw = np.arctan2(b_tang[1], b_tang[0])

  t_pose = node.pose.copy()
  t_pose[:2] = final_position
  t_pose[2] = yaw
  t_node = Node(t_pose)
  center, radius = find_circle(node, t_node)

  start_angle = np.arctan2(node.position[1] - center[1], node.position[0] - center[0])
  end_angle = np.arctan2(final_position[1] - center[1], final_position[0] - center[0])

  diff_ang = end_angle - start_angle

  # update for rrt*
  arc_length = radius * np.abs(diff_ang)
  cost = node.cost + arc_length
  t_node.cost = cost

  valid = True

  angle_tries = np.ceil(arc_length / occupancy_grid.resolution).astype(int)

  # sample equally spaced points on the curve to see if it completely avoids obstacles
  for j in range(angle_tries):
    angle = start_angle + (diff_ang / angle_tries) * j

    x_pos = center[0] + radius * np.cos(angle)
    y_pos = center[1] + radius * np.sin(angle)

    test_pos = np.array([x_pos, y_pos], np.float32)

    if not occupancy_grid.is_free(test_pos):
      valid = False
      break
...
def rrt_star(start_pose, goal_position, occupancy_grid):
  ...
  for _ in range(MAX_ITERATIONS):
    ...
    x_new = adjust_pose(x_nearest, position, occupancy_grid)
    ...
    for x_near in X_near:
      p_x_min = adjust_pose(x_near, x_new.position, occupancy_grid)

      if p_x_min is not None and p_x_min.cost < c_min:
        x_min = x_near
        c_min = p_x_min.cost

    x_new = adjust_pose(x_min, x_new.position, occupancy_grid)

    x_min.add_neighbor(x_new)
    x_new.parent = x_min
    graph.append(x_new)

    if np.linalg.norm(x_new.position - goal_position) < .2:
      final_node = x_new
