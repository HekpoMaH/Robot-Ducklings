...
class SLAM(object):
  ...
  def update(self):
    ...
    if self._tf.frameExists(a) and self._tf.frameExists(b):
      ...
      print('Unable to finddd:', self._tf.frameExists(a), self._tf.frameExists(b))
    pass
...
def run(args):
  ...
  publisher = rospy.Publisher('/' + LEADER + '/cmd_vel', Twist, queue_size=5)
  path_publisher = rospy.Publisher('/' + LEADER + '/path', Path, queue_size=1)
  slam = SLAM()
  goal = GoalPose()
  frame_id = 0
  current_path = []
